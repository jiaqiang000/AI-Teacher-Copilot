# AI Teacher Copilot Install

This file is for coding agents. If the AI Teacher Copilot repository is not already cloned and open, clone `https://github.com/jiaqiang000/AI-Teacher-Copilot.git` first, then continue from the repository root.

## Goal

Bootstrap an AI Teacher Copilot local development workspace on the user's machine with the least risky path available.
...
- The AI Teacher Copilot repository is cloned and the current working directory is the repo root.
- `config.yaml` exists.
...
- The user receives the exact next command to launch AI Teacher Copilot.
...
- If the current directory is not the AI Teacher Copilot repository root, clone `https://github.com/jiaqiang000/AI-Teacher-Copilot.git` if needed, then change into the repository root.
- Confirm the current directory is the AI Teacher Copilot repository root by checking that `Makefile`, `backend/`, `frontend/`, and `config.example.yaml` exist.
...
5. Exact next command to start AI Teacher Copilot
